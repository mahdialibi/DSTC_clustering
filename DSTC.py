...
def autoencoder(dims, act=tf.nn.leaky_relu, init='glorot_uniform', activity_regularizer=l1(10e-6)):
    # sparce autoencoder
    n_stacks = len(dims) - 1
    # input
    ...
    for i in range(n_stacks - 1):
        h = tf.keras.layers.Dense(
            dims[i + 1], activation=act, kernel_initializer=init, activity_regularizer=activity_regularizer, name='encoder_%d' % i)(h)
    h = tf.keras.layers.Dense(
    ...
    """we return the complete autoencoder and the encoder seperately"""
...
class ClusteringLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        """For Step (i), we compute a soft cluster assignment for each data point. Maaten and Hinton
(2008) propose the Student’s t-distribution Q with
a single degree of freedom to measure the similarity between embedded points """
...
class DSTC(object):
    ...
    def fit(self, x, y=None, maxiter=2e4, batch_size=256, tol=1e-3,
        ...
        predicted = self.encoder.predict(x)
        y_pred = kmeans.fit_predict(predicted)
        ...
        print(('saving model to:', save_dir + 'DSTC_model_final.h5'))
        self.model.save_weights(save_dir + 'DSTC_model_final.h5')
        # plot
        ...
        pickle.dump(y_pred, open("ypred.pkl", "wb"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', default='stackoverflow',
                        choices=['stackoverflow', 'big_stackoverflow', 'biomedical', 'search_snippets'])
    ...
    parser.add_argument(
        '--ae_weights', default='/results/ae_weights.h5')
    ...
    elif args.dataset == 'big_stackoverflow':
        args.update_interval = 500
        # args.maxiter = 2200
        args.pretrain_epochs = 12
    ...
    # create model
    ####################################################################################
    dec = DSTC(dims=[x.shape[-1], 1000, 500, 2000, 20], n_clusters=n_clusters)
